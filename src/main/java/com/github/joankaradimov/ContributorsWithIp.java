...
public class ContributorsWithIp {
    ...
    public int getIndex(Contributor contributor) {
        return (contributors.get(contributor) << 1) | CONTRIBUTOR_TYPE;
    }

    public void dump(Path outputDirectory) throws IOException {
        File outputFile = outputDirectory.resolve("contributors_with_ip").toFile();
